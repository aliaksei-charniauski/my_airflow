# Project: my_airflow
# DAG: my_airflow_001_deployment
#
# Date: 2025_09_16
#
# Description: 
# Initial DDL deployment for my_airflow schema in Postgres
...
def get_ddl_files(my_ddl_dir):
    ...
    ddl_filenames = [f for f in os.listdir(my_ddl_dir) if os.path.isfile(os.path.join(my_ddl_dir, f)) and f.startswith('my_airflow_ddl') and f.endswith('.sql')]
...
def my_airflow_001_deployment():
    
    now = datetime.now()
    print(">>> START OF PROCESS %s: %s <<<" % ('my_airflow_001_deployment', now))
...
###########################################
    ...
    now = datetime.now()
    print(">>> END OF PROCESS %s: %s <<<" % ('my_airflow_001_deployment', now))
...
with DAG(
    ...
    task_upload = PythonOperator(
        ...
        python_callable=my_airflow_001_deployment
    )
