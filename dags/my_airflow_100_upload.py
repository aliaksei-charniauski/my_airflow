...
def load_to_postgres(df, filename):
    ...
    if table_name == 'c_bank':

        insert_query = f"""
            INSERT INTO {APP_SCHEMA}.{table_name} (bank_id, bank_name, note)
            VALUES %s
        ...
        data_tuples = [
            (row['bank_id'], row['bank_name'], row['note'])
            for _, row in df.iterrows()
        ]

    elif table_name == 'c_currency_type':

        insert_query = f"""
            INSERT INTO {APP_SCHEMA}.{table_name} (currency_id, currency_name)
            VALUES %s
    ...
    elif table_name == 'c_expense_type':

        insert_query = f"""
            INSERT INTO {APP_SCHEMA}.{table_name} (expense_type_id, expense_type_name)
            VALUES %s
        ...
        data_tuples = [
            (row['expense_type_id'], row['expense_type_name'])
            for _, row in df.iterrows()
        ]

    elif table_name == 'c_family':
        
        insert_query = f"""
            INSERT INTO {APP_SCHEMA}.{table_name} (family_id, family_name)
            VALUES %s
    ...
    elif table_name == 'm_bank_coverage':

        insert_query = f"""
            INSERT INTO {APP_SCHEMA}.{table_name} (bank_coverage_id, bank_id, coverage_name)
            VALUES %s
            ON CONFLICT (bank_coverage_id) DO NOTHING
        """
        row_hash = f""" """

        data_tuples = [
            (row['bank_coverage_id'], row['bank_id'], row['coverage_name'])
            for _, row in df.iterrows()
        ]
        
    elif table_name == 'r_expense':
...
def my_airflow_100_upload():
    

    now = datetime.now()
    print(">>> START OF PROCESS %s: %s <<<" % ('my_airflow_100_upload', now))
...
###########################################
    ...
    now = datetime.now()
    print(">>> END OF PROCESS %s: %s <<<" % ('my_airflow_100_upload', now))
...
#    schedule_interval='@daily',  # or None for manual runs
    ...
    task_upload = PythonOperator(
        ...
        python_callable=my_airflow_100_upload
    )
